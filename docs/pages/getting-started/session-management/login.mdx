...
To signin your users using a SvelteKit form action, we can use the `SignIn` component exported from `@oneum-io/sveltekit/components`.
...
Client-side is a bit simpler as we just need to import a button `on:click` handler from `@oneum-io/sveltekit/client`.
...
To use the SvelteKit form action for signing out, we can use the `SignOut` component exported from `@oneum-io/sveltekit/components`.
...
<script lang="ts">
  import { signOut } from "@oneum-io/sveltekit/client"
</script>
